...
import lfc.chat01.lfcchat01.chat01.UserMapper;
import lfc.chat01.lfcchat01.chat01.UserModel;
import lfc.chat01.lfcchat01.chat01.UserUtil;
...
public class UserMapperTest {
    ...
    public void update() throws Exception {
        ...
        Long userId1 = 2L;
        Integer count = UserUtil.callMapper(UserMapper.class,
                mapper -> mapper.update(UserModel.builder().id(userId1).name("ready").build()));
        log.info("更新行数：{}", count);

        log.info("---------------------");
        // 将userId=3的name修改为：路人,薪水为：1000.88
        Long userId2 = 3L;
        count = UserUtil.callMapper(UserMapper.class,
                mapper -> mapper.update(UserModel.builder().id(userId2).name("ready").salary(1000.88D).build()));
        log.info("更新行数：{}", count);
    }
